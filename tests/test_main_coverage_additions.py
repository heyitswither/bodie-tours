...
def test_tc_m05_get_m365_token_missing_token_in_json(mock_post, mock_db):
    ...
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"refresh_token": "new_rtoken"}
    mock_post.return_value = mock_response
...
def test_tc_m07_check_m365_availability_http_failure(mock_get, mock_db):
    ...
    with pytest.raises(
        Exception, match="Failed to query M365 calendar: Internal Error"
    ):
...
def test_tc_m08_check_m365_availability_mismatched_criteria(mock_get, mock_db):
    ...
    assert (
        main.check_m365_availability("token", "user_id", "2026-06-15", "10:00") is False
    )
...
def test_tc_m09_check_m365_availability_tz_fallbacks(mock_get, mock_db):
    ...
    mock_get.return_value = mock_response
    assert (
        main.check_m365_availability("token", "user_id", "2026-06-15", "10:00") is True
    )
...
def test_tc_m11_inject_m365_event_failure(mock_post, mock_db):
    ...
    with pytest.raises(Exception, match="Failed to inject M365 event: Injection Error"):
        main.inject_m365_event(
            ...
            "2026-06-15",
            "10:00",
            {"name": "Alice", "phone": "123", "party_size": 2},
            "booking_id",
        )
...
def test_tc_m12_get_qbo_token_missing_auth(mock_db):
    ...
    with pytest.raises(
        Exception, match="QBO Auth configuration missing. Run OAuth flow first."
    ):
...
def test_tc_m14_get_qbo_token_missing_credentials(mock_db):
    ...
    with patch.dict(os.environ, {}, clear=True):
        with pytest.raises(
            Exception, match="Missing QBO credentials for token refresh."
        ):
            main.get_qbo_access_token()
...
def test_tc_m16_create_qbo_invoice_production(mock_post):
    ...
    mock_response.json.return_value = {"Invoice": {"Id": "prod_inv_123"}}
    mock_post.return_value = mock_response
...
def test_create_qbo_invoice_sandbox(mock_post):
    mock_response = MagicMock()
    mock_response.status_code = 201
    mock_response.json.return_value = {"Invoice": {"Id": "sandbox_inv_123"}}
    mock_post.return_value = mock_response
...
def test_tc_m17_create_qbo_invoice_failure(mock_post):
    ...
    with pytest.raises(
        Exception, match="Failed to create QBO invoice: Invalid customer"
    ):
...
def test_tc_m19_process_booking_inventory_exists_not_booked(mock_db):
    ...
    mock_snapshot.to_dict.return_value = {"taken_slots": []}
...
def test_tc_m20_process_booking_slot_already_booked(mock_db):
    ...
    dt_local = datetime.strptime("2026-06-15 10:00", "%Y-%m-%d %H:%M").replace(
        tzinfo=local_tz
    )
    dt_local_utc = dt_local.astimezone(timezone.utc)

    mock_snapshot.to_dict.return_value = {"taken_slots": [dt_local_utc]}
    inventory_ref.get.return_value = mock_snapshot

    with pytest.raises(
        ValueError, match="This time slot is already booked by another group."
    ):
        main.process_booking_transaction(
            transaction, inventory_ref, "2026-06-15", "10:00", 5, {"name": "Alice"}
        )
...
def test_tc_m21_handle_booking_cors_origins():
    ...
    for origin in origins:
        req = make_mock_post_request(
            headers={"Origin": origin}, json_data={"date": "invalid-date"}
        )
    ...
    req = make_mock_post_request(
        headers={"Origin": "https://malicious.com"}, json_data={"date": "invalid-date"}
    )
    response, status_code, resp_headers = main.handle_booking(req)
    assert status_code == 409
    assert (
...
def test_tc_m22_handle_booking_options_preflight():
    builder = EnvironBuilder(
        method="OPTIONS", headers={"Origin": "https://bodiefoundation.org"}
    )
    ...
    assert response == ""
    assert (
        resp_headers.get("Access-Control-Allow-Origin") == "https://bodiefoundation.org"
    )
    assert resp_headers.get("Access-Control-Allow-Methods") == "POST, GET"


def test_tc_m23_handle_booking_invalid_date_format():
    req = make_mock_post_request(json_data={"date": "2026/06/15"})
    response, status_code, resp_headers = main.handle_booking(req)
    assert status_code == 409
...
def test_tc_m24_handle_booking_invalid_time_format():
    req = make_mock_post_request(json_data={"date": "2026-06-15", "time": "10-00"})
    response, status_code, resp_headers = main.handle_booking(req)
    assert status_code == 409
...
def test_tc_m25_handle_booking_rollback_success(
    ...
    mock_inventory_snapshot.to_dict.return_value = {"taken_slots": [dt_local_utc]}
...
def test_tc_m25_handle_booking_rollback_time_not_in_slots(
    ...
    mock_inventory_snapshot = MagicMock()
    mock_inventory_snapshot.exists = True
    mock_inventory_snapshot.to_dict.return_value = {"taken_slots": []}
...
def test_tc_m31_m365_callback_invalid_state():
    builder = EnvironBuilder(
        method="GET", query_string={"code": "123", "state": "stateA"}
    )
    req = Request(builder.get_environ())
...
def test_tc_m33_m365_callback_success(mock_post, mock_db):
    ...
    builder = EnvironBuilder(
        method="GET", query_string={"code": "auth_code", "state": "stateA"}
    )
    req = Request(builder.get_environ())
    req.cookies = {"m365_oauth_state": "stateA"}
...
def test_tc_m35_m365_callback_db_error(mock_post, mock_db):
    ...
    mock_doc = MagicMock()
    mock_db.collection.return_value.document.return_value = mock_doc
    mock_doc.get.return_value.to_dict.return_value = {"tenant_id": "my_tenant"}
...
def test_tc_m39_qbo_webhook_verifier_token_types(mock_db):
    ...
    tokens_to_test = ["my_string_token", b"my_bytes_token", 123456]
...
def test_tc_m42_qbo_webhook_missing_entity_id(mock_db):
    ...
    payload = {
        "eventNotifications": [
            {
                "dataChangeEvent": {
                    "entities": [{"name": "Invoice", "operation": "Update"}]
                }
            }
        ]
    }
...
def test_firestore_client_initialization_failure():
    ...
    with patch(
        "google.cloud.firestore.Client", side_effect=Exception("Initialization error")
    ):
        try:
...
def test_firestore_client_dummy_db_full_coverage():
    ...
    try:
        ...
        mock_response = MagicMock()
        mock_response.status_code = 201
        mock_response.json.return_value = {"Invoice": {"Id": "mock_qbo_invoice_id"}}
        with patch("requests.post", return_value=mock_response):
            ...
            evt = main.inject_m365_event(
                m_tok, m_usr, "2026-06-15", "10:00", {}, "b_123"
            )
            ...
            try:
                ...
                assert (
                    str(e)
                    == "Booking conflict: The requested time slot is already taken."
                )
...
def test_qbo_credentials_resolution_coverage():
    ...
    auth_env_fallback = {"environment": "production"}
    with patch.dict(
        os.environ,
        {
            "QBO_CLIENT_ID": "env_id_val",
            "QBO_CLIENT_SECRET": "env_secret_val",
            "QBO_REDIRECT_URI": "env_redirect_val",
        },
    ):
...
def test_process_booking_taken_slots_coverage():
    ...
    local_tz = ZoneInfo("America/Los_Angeles")
    requested_dt = datetime.strptime("2026-06-15 10:00", "%Y-%m-%d %H:%M").replace(
        tzinfo=local_tz
    )
    ...
    mock_snapshot = MagicMock()
    mock_snapshot.exists = True
    mock_snapshot.to_dict.return_value = {"taken_slots": [requested_dt.isoformat()]}
    inventory_ref.get.return_value = mock_snapshot
    with pytest.raises(
        ValueError, match="This time slot is already booked by another group."
    ):
    ...
    mock_snapshot.to_dict.return_value = {"taken_slots": [naive_dt_matching]}
    with pytest.raises(
        ValueError, match="This time slot is already booked by another group."
    ):
        main.process_booking_transaction(
    ...
    mock_snapshot.to_dict.return_value = {"taken_slots": [requested_dt]}
    with pytest.raises(
        ValueError, match="This time slot is already booked by another group."
    ):
        main.process_booking_transaction(
