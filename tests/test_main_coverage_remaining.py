...
def test_m365_free_availability_branches(mock_get, mock_token, mock_main_db):
    ...
    builder = EnvironBuilder(method='GET', query_string="start=2026-06-15&end=2026-06-15")
    request = Request(builder.get_environ())
        ...
        "value": [
            {
                ...
                "showAs": "free",
                "start": {"dateTime": "2026-06-15T09:00:00", "timeZone": "Pacific Standard Time"},
                "end": {"dateTime": "2026-06-15T10:00:00", "timeZone": "Pacific Standard Time"}
            }
        ]
    }
    ...
    mock_doc.to_dict.return_value = {
        "taken_slots": [datetime(2026, 6, 15, 16, 0, tzinfo=timezone.utc)] # 16:00 UTC = 09:00 PDT
    }
...
def test_cancel_tour_branches(mock_delete, mock_main_db):
    ...
    builder = EnvironBuilder(method='OPTIONS', headers={"Origin": "https://www.bodiefoundation.org"})
    request = Request(builder.get_environ())
    ...
    mock_main_db.collection.return_value.document.return_value.get.return_value = mock_booking_snap
    body, status, headers = main.cancel_tour(request)
    assert status == 404
    ...
    mock_booking_snap.to_dict.return_value = {
        ...
        "integration_ids": {
            "m365_event_id": "event_123",
            "qbo_invoice_id": "inv_123"
        }
    }
    mock_main_db.collection.return_value.document.return_value.get.return_value = mock_booking_snap
...
def test_qbo_webhook_branches(mock_db):
    ...
    import hmac
    import hashlib
    computed = hmac.new(verifier_token.encode('utf-8'), payload_str.encode('utf-8'), hashlib.sha256).digest()
...
def test_send_booking_receipt_email_branches(mock_post, mock_token, mock_main_db):
    ...
    mock_main_db.collection.return_value.document.return_value.get.return_value = mock_tmpl_doc
...
@patch('main.get_m365_access_token')
@patch('requests.post')
def test_send_booking_receipt_email_additional_gaps(mock_post, mock_token, mock_main_db):
    mock_token.return_value = ("m_token", "m_user")
    ...
    mock_main_db.collection.return_value.document.return_value.get.side_effect = Exception("Firestore read error")
    with patch('main.db', mock_client_db):
        res = main.send_booking_receipt_email("booking_123", booking_data_naive_dt)
...
def test_qbo_login_config_exception(mock_db):
    ...
    assert resp.status_code == 302 # Redirect still succeeds using environment fallbacks
...
def test_qbo_callback_failures(mock_post, mock_db):
    ...
    mock_auth_doc.get.return_value.to_dict.return_value = {
        "client_id": "cid", "client_secret": "csec", "redirect_uri": "http://callback"
    }
    ...
    import requests
    mock_resp.raise_for_status.side_effect = requests.HTTPError("Bad request token exchange")
    ...
    mock_resp_success.json.return_value = {"access_token": "tok"} # missing refresh_token
...
def test_m365_login_branches(mock_db):
    mock_auth_doc = MagicMock()
    mock_auth_doc.get.return_value.to_dict.return_value = {
        "client_id": "cid", "redirect_uri": "http://callback"
    }
    mock_db.collection.return_value.document.return_value = mock_auth_doc
...
def test_m365_free_availability_exception_branches(mock_get, mock_token, mock_main_db):
    ...
    resp, status = main.m365_free_availability(request)
    assert status == 200 # Returns empty results since it catches and falls back to empty events
...
def test_cancel_tour_additional_gaps(mock_delete, mock_main_db):
    ...
    mock_main_db.collection.return_value.document.return_value.update.side_effect = update_side_effect
    with patch('main.get_m365_access_token', return_value=("tok", "usr")):
        body, status, headers = main.cancel_tour(request)
        assert status == 200 # Still returns 200 since the inventory release exception is caught and ignored
...
def test_check_m365_availability_direct(mock_get, mock_main_db):
    ...
    with patch('main.db', mock_client_db):
        ...
        res = main.check_m365_availability("token", "user_1", "2026-06-15", "10:00", "cal_123")
        assert res is True
        args, kwargs = mock_get.call_args
        assert "calendars/cal_123" in args[0]
        
        # 2. Without calendar_id (covers line 180)
        res = main.check_m365_availability("token", "user_1", "2026-06-15", "10:00", None)
        assert res is True
        args, kwargs = mock_get.call_args
...
def test_inject_m365_event_direct(mock_post, mock_main_db):
    ...
    with patch('main.db', mock_client_db):
        ...
        eid = main.inject_m365_event("token", "user_1", "2026-06-15", "10:00", guest_data, "booking_1", "cal_123")
        assert eid == "event_999"
        args, kwargs = mock_post.call_args
        assert "calendars/cal_123" in args[0]
        
        # 2. Without calendar_id (covers line 252)
        eid = main.inject_m365_event("token", "user_1", "2026-06-15", "10:00", guest_data, "booking_1", None)
        assert eid == "event_999"
        args, kwargs = mock_post.call_args
        assert "calendars" not in args[0]
...
def test_resolve_qbo_credentials_production_missing_redirect_uri():
    ...
    with patch.dict(os.environ, {}, clear=True), patch('os.environ.get', return_value=None):
        with pytest.raises(ValueError, match="Redirect URI must be configured for QBO in production environment"):
...
@patch('main.get_m365_access_token')
@patch('requests.get')
def test_m365_free_availability_legacy_slots_and_timestamps(mock_get, mock_token, mock_main_db):
    mock_token.return_value = ("token", "ranger_1")
    builder = EnvironBuilder(method='GET', query_string="start=2026-06-15&end=2026-06-15")
    request = Request(builder.get_environ())
    ...
    mock_doc.to_dict.return_value = {
        "slots": [mock_ts] # list of timestamps
    }
    with patch('main.db', mock_client_db):
        resp, status = main.m365_free_availability(request)
        assert status == 200
...
def test_cancel_tour_invalid_token_branch(mock_main_db):
    ...
    builder = EnvironBuilder(method='GET', query_string="booking_id=b_123&token=token_mismatch")
    request = Request(builder.get_environ())
