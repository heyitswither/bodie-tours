...
def mock_env(monkeypatch):
    ...
    class FixedDatetime(dt_mod.datetime):
        @classmethod
        def now(cls, tz=None):
            return dt_mod.datetime(2026, 6, 9, 12, 0, 0, tzinfo=tz or dt_mod.timezone.utc)
...
def test_successful_retry_and_email(monkeypatch):
    ...
    monkeypatch.setattr(firestore, "Client", lambda **kwargs: mock.MagicMock(collection=lambda name: mock_collection))
    # Mock external helpers
    monkeypatch.setattr("retry_unpaid_bookings.get_qbo_access_token", lambda: ("qbo_token", "realm"))
    monkeypatch.setattr("retry_unpaid_bookings.create_qbo_invoice", lambda token, realm, size, guest: ("inv123", "https://pay.example.com"))
    monkeypatch.setattr("retry_unpaid_bookings.get_m365_access_token", lambda: ("m365_token", "user_id"))
    monkeypatch.setattr("retry_unpaid_bookings._send_temp_issue_email", lambda *args, **kwargs: True)
    ...
    assert any("retry_attempts" in upd and upd["retry_attempts"] == 2 for upd in update_payloads)
...
def test_skip_due_to_max_retries(monkeypatch):
    ...
    monkeypatch.setattr("retry_unpaid_bookings.get_qbo_access_token", lambda: (_ for _ in ()).throw(RuntimeError()))
    result, status = retry_unpaid_bookings(None)
    assert status == 200
