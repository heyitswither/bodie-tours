...
def test_get_m365_token_expired_http_failure(mock_post, mock_db):
    ...
    with pytest.raises(
        Exception, match="Failed to refresh M365 token: Internal Server Error"
    ):
        prune_unpaid_slots._get_m365_token_for_prune()
...
def test_prune_unpaid_slots_reminders(mock_send, mock_get_token, mock_db):
    ...
    with patch("prune_unpaid_slots.datetime") as mock_dt:
        ...
        mock_doc1.reference.update.assert_called_once_with(
            {"reminder_sent": "increment_sentinel"}
        )
        mock_doc3.reference.update.assert_called_once_with(
            {"reminder_sent": "increment_sentinel"}
        )
...
def test_prune_unpaid_slots_cancellation_cases(
    mock_remove_event, mock_process, mock_get_token, mock_db
):
    mock_get_token.return_value = ("token", "ranger_1")
    now = datetime(2026, 6, 15, 12, 0, tzinfo=timezone.utc)
...
def test_process_cancellation_taken_slots_coverage():
    ...
    slot_dt = datetime.strptime("2029-06-03 18:00", "%Y-%m-%d %H:%M").replace(
        tzinfo=local_tz
    )
    ...
    ts_nonmatching_dt_tz = datetime.strptime(
        "2029-06-03 19:00", "%Y-%m-%d %H:%M"
    ).replace(tzinfo=local_tz)
    ts_matching_dt_naive = slot_dt_utc.replace(tzinfo=None)
    ts_nonmatching_dt_naive = (
        datetime.strptime("2029-06-03 19:00", "%Y-%m-%d %H:%M")
        .replace(tzinfo=local_tz)
