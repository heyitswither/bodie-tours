...
def test_prune_unpaid_slots_remove_event_exception(mock_get_token, mock_db):
    ...
    with patch(
        "prune_unpaid_slots.process_cancellation_transaction", return_value=True
    ), patch(
        "prune_unpaid_slots.remove_m365_event",
        side_effect=Exception("API limit exceeded"),
    ), patch(
        "prune_unpaid_slots.datetime"
    ) as mock_dt:
...
def test_send_outlook_reminder_custom_template_branches(mock_db):
    ...
    with patch.dict(
        os.environ, {"EMAIL_TEMPLATE_TYPE": "custom", "TOUR_PRICE_PER_PERSON": "30.00"}
        ...
        assert (
            json_payload["message"]["subject"] == "Custom subject id_123 with party 3"
        )
        assert (
            json_payload["message"]["body"]["content"]
            == "Custom body Name 2026-06-15 10:00 link http://pay.me total 90.00"
        )
    ...
    with patch.dict(os.environ, {"EMAIL_TEMPLATE_TYPE": "custom"}), patch(
        "requests.post"
    ) as mock_post:
        mock_post.return_value.status_code = 202
        res = prune_unpaid_slots.send_outlook_reminder(
