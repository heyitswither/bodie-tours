...
class MockFirestore:
    def __init__(self):
        ...
        self.db["config/qbo_auth"] = {
            "access_token": "qa", "refresh_token": "qr", "realmId": "qrealm"
        }
...
def setup_post_mock(mock_requests_post, is_available=True):
    ...
    def post_se(*args, **kwargs):
        ...
        if "token" in url:
            resp.json.return_value = {"access_token": "mock", "refresh_token": "mock", "expires_in": 3600}
        elif "/events" in url:
...
def test_booking_no_m365_availability(client, mock_requests_post, mock_requests_get, patch_firestore):
    setup_post_mock(mock_requests_post)
...
def test_qbo_token_refresh_during_booking(client, mock_requests_post, patch_firestore):
    ...
    resp = client.get(f'/qbo/callback?code=mockcode&realmId=123&state={state_cookie}',
