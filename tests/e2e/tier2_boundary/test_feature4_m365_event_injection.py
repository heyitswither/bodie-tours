...
def test_m365_inject_empty_subject(client, mock_requests):
    response = client.post(
        "/m365_event_injection", json={"subject": "", "start": "2026-06-15T09:00:00"}
    )
...
def test_m365_inject_max_length_description(client, mock_requests):
    long_desc = "A" * 10000
    response = client.post(
        "/m365_event_injection", json={"subject": "Tour", "description": long_desc}
    )
    assert response.status_code in [200, 400, 500]
...
def test_m365_inject_invalid_timezone_id(client, mock_requests):
    response = client.post(
        "/m365_event_injection",
        json={"subject": "Tour", "timezone": "Invalid/Timezone"},
    )
    assert response.status_code in [200, 400, 500]
...
def test_m365_inject_500_retry(client, mock_requests):
    ...
    response = client.post(
        "/m365_event_injection",
        json={"subject": "Tour", "start": "2026-06-15T09:00:00"},
    )
    assert response.status_code in [200, 400, 500]
